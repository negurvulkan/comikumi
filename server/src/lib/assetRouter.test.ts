...
describe("POST /api/fonts (upload)", () => {
  it("rejects a file with a disallowed extension", async () => {
    const res = await api.post("/api/fonts").attach("font", Buffer.from("not a font"), "evil.exe");
  ...
  it("uploads to the global dir when no project assetsDir is configured, and lists it back with family derived from the filename", async () => {
    const res = await api.post("/api/fonts").attach("font", Buffer.from([0, 1, 2, 3]), "MyFont.ttf");
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ ok: true, fileName: "MyFont.ttf", scope: "global" });

    const list = await api.get("/api/fonts");
  ...
  it("serves the uploaded file's exact bytes back via GET /file/:fileName", async () => {
    ...
    await api.post("/api/fonts").attach("font", bytes, "Roundtrip.ttf");
    const res = await api.get("/api/fonts/file/Roundtrip.ttf");
    expect(res.status).toBe(200);
    expect(Buffer.compare(res.body, bytes)).toBe(0);
  });

  it("rejects an unsafe file name in the serve route", async () => {
    const res = await api.get("/api/fonts/file/..%2F..%2Fetc%2Fpasswd.ttf");
    expect(res.status).toBe(400);
  });
});
...
describe("project-specific assetsDir: merge + collision", () => {
  ...
  it("uploads go to the project dir once assetsDir is configured, scoped 'project'", async () => {
    ...
    const upload = await api.post("/api/fonts").attach("font", Buffer.from([1]), "ProjectOnly.ttf");
  ...
  it("a project-scoped file with the same name as a global one wins, in both listing and serving", async () => {
    ...
    await api.put("/api/settings").send({ scanRoot: env.scanRoot, assetsDir: "" });
    await api.post("/api/fonts").attach("font", globalBytes, "Collide.ttf");
    await api.put("/api/settings").send({ scanRoot: env.scanRoot, assetsDir: projectAssetsDir() });
    await api.post("/api/fonts").attach("font", projectBytes, "Collide.ttf");
    ...
    const served = await api.get("/api/fonts/file/Collide.ttf");
